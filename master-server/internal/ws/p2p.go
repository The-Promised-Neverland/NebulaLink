...
func (p *P2PManager) StartP2PTransfer(requestingAgent, sourceAgent string) (string, error) {
	...
	p.mu.Unlock()
	go p.manageRetries(ctx, connectionID, requestingEndpoint, sourceEndpoint)
	return connectionID, nil
}

// manageRetries handles retry logic with exponential backoff
func (p *P2PManager) manageRetries(ctx context.Context, connectionID, requestingEndpoint, sourceEndpoint string) {
	defer func() {
		p.mu.Lock()
	...
	p.switchToRelay(connectionID)
}

// sendP2PInitiation sends P2P initiation message to both agents
func (p *P2PManager) sendP2PInitiation(connectionID, requestingAgent, sourceAgent, requestingEndpoint, sourceEndpoint string, attemptNumber int) error {
...
func (p *P2PManager) switchToRelay(connectionID string) {
	p.mu.RLock()
	state := p.activeTransfers[connectionID]
	p.mu.RUnlock()
	if state == nil {
	...
	if sourceConn != nil {
		...
		relayMsg := models.Message{
			Type: models.MasterMsgSwitchToRelay,
			Payload: map[string]interface{}{
				"connection_id":        connectionID,
...
func (p *P2PManager) GetTransferState(connectionID string) *P2PTransferState {
	p.mu.RLock()
	defer p.mu.RUnlock()
